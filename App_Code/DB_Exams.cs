...
AND
( ISNULL(ASS.USER_FINISH, '') = ''  OR convert(datetime, ASS.USER_FINISH, 20)   >=  convert(datetime, @STARTDATE, 20) )
AND
( ISNULL(ASS.USER_FINISH, '') = ''  OR convert(datetime, ASS.USER_FINISH, 20)   <= convert(datetime, @FINISHDATE, 20)  )
AND 
ASS.TRAINEE_ID = CASE WHEN @USE_TRAINEEID = 1 THEN @TRAINEEID ELSE ASS.TRAINEE_ID END
AND 
1 = CASE WHEN @PASSFAILNOSHOW IN ( 'UNFINISHED', 'NOSHOW', 'ALL' ) THEN 1 
         WHEN @PASSFAILNOSHOW IN ('PASSED', 'FAILED') 
                AND CAST(ASS.GRADE AS DECIMAL(10, 3)) >= CAST(@GRADE_START AS DECIMAL(10, 3))
                AND CAST(ASS.GRADE AS DECIMAL(10, 3)) <= CAST(@GRADE_FINISH AS DECIMAL(10, 3))
            THEN 1
         ELSE 0 END
AND 
...
AND
 1 = CASE WHEN @PASSFAILNOSHOW = 'ALL' THEN 1
         WHEN @PASSFAILNOSHOW = 'UNFINISHED' AND ASS.STATUS IN ('USER_STARTED', 'ASSIGNED') THEN 1
         WHEN @PASSFAILNOSHOW = ASS.STATUS THEN 1
         ELSE 0 END 
